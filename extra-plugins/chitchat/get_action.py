...
def getStarCount(list):
    ...
    if len(list) == 1 and list[0] == 'umbobot':
        return 0
...
def refreshData(dataList, tab_index):
    ...
    credentials = ServiceAccountCredentials.from_json_keyfile_name('UmboBot.json', scope)
    gc = gspread.authorize(credentials)

    inti_col = 0

    if tab_index == 0:
        wks = gc.open('UmboBot Brain').get_worksheet(tab_index)
        inti_col = 1
    else:
        wks = gc.open('UmboBot Brain').get_worksheet(tab_index)
        inti_col = 0
